...
describe("Color Utils", function()
	...
	it("should return color value when receiving oklch", function()
		local hex_value = utils.get_color_value("oklch(0 0 0)")
		assert.are.equal(hex_value, "#000000")
	end)

	it("should return color value when receiving oklch white", function()
		local hex_value = utils.get_color_value("oklch(1 0 0)")
		assert.are.equal(hex_value, "#FFFFFF")
	end)

	it("should return oklch values with percentage lightness", function()
		local vals = utils.get_oklch_values("oklch(70% 0.15 200)")
		assert.are.equal(vals[1], 0.7)
		assert.are.equal(vals[2], 0.15)
		assert.are.equal(vals[3], 200)
	end)

	it("should return oklch values with deg suffix", function()
		local vals = utils.get_oklch_values("oklch(0.5 0.2 180deg)")
		assert.are.equal(vals[1], 0.5)
		assert.are.equal(vals[2], 0.2)
		assert.are.equal(vals[3], 180)
	end)

	it("should return oklch values with turn suffix", function()
		local vals = utils.get_oklch_values("oklch(0.5 0.2 0.5turn)")
		assert.are.equal(vals[1], 0.5)
		assert.are.equal(vals[2], 0.2)
		assert.are.equal(vals[3], 180)
	end)
